...
- JWT refresh token, HttpOnly cookie, 소셜 로그인 등 인증 보안 고도화
...
- 첫 화면은 로그인 진입 화면으로 구성하고, 로그인 이후 바로 가이드북 피드로 이동하도록 구성했습니다. 사용자별 인쇄목록, 관심목록, 관리자 권한을 분리하기 위함입니다.
...
- 가이드북 생성은 국가/도시 선택, 지도 위치 포인트, 사진+텍스트 블록을 순서대로 입력하고 우측에서 미리보는 구조로 만들었습니다. 자유도가 너무 높은 편집기는 과제 범위에서 복잡도가 커지기 때문에, 노션처럼 위에서 아래로 쌓는 구조를 선택했습니다.

### 의도적으로 넣지 않은 것

- 실제 결제, 배송, 인쇄 API 호출은 구현하지 않았습니다. 과제 조건과 포트폴리오 범위에 맞춰 주문 생성, 판매목록 반영, 관리자 주문 상태 변경까지 구현했습니다.
- JWT refresh token, HttpOnly cookie 기반 세션, 소셜 로그인은 구현하지 않았습니다. 현재는 JWT access token 기반으로 로그인 사용자와 관리자 권한을 검증하며, 실서비스 확장 시 세션 보안을 강화할 수 있습니다.
- 실제 영상 분석 AI API는 연결하지 않았습니다. 가이드북 생성 폼에서 영상 링크를 입력하는 구조와 분석 결과가 들어갈 위치만 구성했습니다.
- Geoapify 지도 API는 선택적으로 사용합니다. API 키가 없을 때도 프로젝트 내부 fallback 지도 이미지로 기능을 확인할 수 있습니다.
