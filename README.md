...
사용자가 메인 탐색에서 상세 확인, 담기, 인쇄 장바구니, 크리에이터 화면, 생성 모달로 이어지는 흐름을 끊기지 않게 사용할 수 있도록 상단 공용 탭바와 반응형 UI를 구성했습니다. 모바일화면 같은 다른 비율의 화면에서도 레이아웃이 깨지지 않도록 media query로 레이아웃을 분기했습니다.
...
- 인쇄 장바구니는 별도 페이지로 분리했습니다. ui 흐름상 마지막부분 비즈니스 로직에서 제일 핵심적인 부분에 해당하기때문에 눈에 잘띄는 위치에 별도분리가 필요했습니다.
